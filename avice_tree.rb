...
class MediaContainer < MediaObject
	...
	def getDataForList(child_array, offset, max, propertiesReq)
		...
		puts Time.now.to_s + ":getDataForList #{@displayname} #{offset} #{highest}, request properties #{propertiesReq}"
		#binding.pry
	...
	dbus_interface PROPERTIES_IFACE do
		dbus_method :Get, "in iface:s, in name:s, out value:v" do |iface, name|
			...
			rvalue = Array.new	
			puts  Time.now.to_s + ":GetAll for #{@displayname} #{iface} #{name} called"
			case iface
			when OBJECT_IFACE
		...
		dbus_method :GetAll, "in iface:s, out values:a{sv}" do |iface|
			...
			rvalues = Hash.new
			puts  Time.now.to_s + ":GetAll for #{@displayname} #{iface} called"
			#binding.pry
			case iface
			when OBJECT_IFACE
	...
	dbus_interface CONTAINER_IFACE do
		...
		dbus_method :ListChildren, "in offset:u, in max:u, in filter:as, out values:aa{sv}" do |offset, max, filter|
			#puts Time.now.to_s + ":ListChildren called for " + @displayName
